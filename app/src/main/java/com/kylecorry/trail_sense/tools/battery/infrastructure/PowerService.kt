...
class PowerService {
    ...
    fun getRates(readings: List<BatteryReadingEntity>, minDuration: Duration, hasCapacity: Boolean): List<Float> {
        if (readings.size < 2) {
            return emptyList()
        }
        ...
        val grouped = mutableListOf<Pair<Boolean, MutableList<BatteryReadingEntity>>>()
        ...
        for (i in 1 until sorted.size) {
            ...
            val group = if (last == null || last.first != charging) {
                val newGroup = Pair(charging, mutableListOf<BatteryReadingEntity>())
    ...
    fun getRate(
        first: BatteryReadingEntity,
        second: BatteryReadingEntity,
    ...
    fun wasCharged(first: BatteryReadingEntity, second: BatteryReadingEntity): Boolean {
     ...
     */
    fun getMaxCapacity(readings: List<BatteryReadingEntity>): Float? {
