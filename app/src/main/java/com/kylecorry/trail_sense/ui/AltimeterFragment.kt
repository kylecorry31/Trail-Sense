...
class AltimeterFragment : Fragment(), Observer {
    ...
    private fun updateAltitude() {
        ...
        val pressureHistory = PressureHistoryRepository.getAll(context!!).filter { Duration.between(it.time, Instant.now()) < CHART_DURATION }.toMutableList()
        pressureHistory.add(
    ...
    private fun getAltitudeHistory(pressureHistory: List<PressureAltitudeReading>): List<AltitudeReading> {
        val altitudeCalculator = AltitudeCalculatorFactory(context!!).create()
        return altitudeCalculator.convert(pressureHistory)
    }
