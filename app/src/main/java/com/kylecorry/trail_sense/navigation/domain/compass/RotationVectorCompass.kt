 ...
 * A compass sensor
 */
class RotationVectorCompass (ctx: Context) : ICompass, ISensor, SensorEventListener, Observable() {
    ...
    override fun start(){
        if (started) return
        sensorManager.getDefaultSensor(Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR)?.also { orientation ->
    ...
    override fun onSensorChanged(event: SensorEvent) {
        ...
        _azimuth = Math.toDegrees(orientation[0].toDouble()).toFloat()
