...
class CsvIOService(private val uriPicker: UriPicker, private val uriService: UriService) :
    ...
    override suspend fun import(): List<List<String>>? {
        val uri = uriPicker.open(listOf("text/csv")) ?: return null
        val csvString = uriService.read(uri) ?: return null
        return CSVConvert.parse(csvString)
    }
