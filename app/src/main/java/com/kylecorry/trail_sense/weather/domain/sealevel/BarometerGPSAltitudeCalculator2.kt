...
internal class BarometerGPSAltitudeCalculator2(private val maxNaturalPressureChange: Float = 3f) :
    IAltitudeCalculator {
    override fun convert(readings: List<PressureAltitudeReading>): List<AltitudeReading> {
        ...
        return readings.map {
            if (it != lastReading) {
                ...
                altitude = if (abs(dp) > maxNaturalPressureChange || it.altitude == lastReading.altitude) {
                    // Barometer is detecting an altitude change or the GPS didn't provide a new altitude
                    val barometerAltitude = altitude + getAltitudeChange(lastReading.pressure, it.pressure)

                    if (abs(it.altitude - barometerAltitude) < MAX_ALTITUDE_DIFF) {
                        it.altitude
                    } else {
                        barometerAltitude
                    }
                } else {
                    // Altitude most likely didn't change
                    altitude * 0.6f + it.altitude * 0.4f
                }
            }
            lastReading = it
            AltitudeReading(
